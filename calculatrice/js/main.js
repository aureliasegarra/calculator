function Calculator()
{
    ...
    this.run    = function()
    {
        $(this.button).click(function(){
            var value = $(this).html();
            // En dessous : pour enlever le zero devant les chiffres
            if (that.init == false)
            {
                $(that.field).val("");
                that.init = true;
            }
            // Permet de ne pas afficher le signe "=" puis
            // Pour Afficher une suite de chiffres tapés sur le clavier de la calculatrice
            if (value != "=")
    ...
    {
        var numbers = $(this.field).val().split(symbol),result;
